...
```bash
pnpm --filter doopx-examples-agents smoke
```
...
```bash
pnpm --filter doopx-examples-agents smoke -- --effort xhigh
```
...
```bash
pnpm --filter doopx-examples-agents standalone
```
...
```bash
pnpm --filter doopx-examples-agents lmstudio-tools
```
...
```bash
pnpm --filter doopx-examples-agents pause-resume
```
...
```bash
pnpm --filter doopx-examples-agents ask-user            # prompts in a terminal
printf "Rust\nmy-app\n" | pnpm --filter doopx-examples-agents ask-user   # one answer per line
```

A missing answer takes the first option (or `doopx-demo` for the name).
...
```bash
pnpm --filter doopx-examples-agents steer
```
