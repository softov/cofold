...
- **commander** is ubiquitous, is on every machine already, and has fifteen years
  of edge cases in its test suite. For a script with four flags, use it.
- **yargs** has richer built-in coercion and a middleware ecosystem.
- **oclif** ships the release machinery too - `oclif pack`, autoupdate, plugin
  install. If you want that, you want oclif; it is not what this is.
- **clipanion**'s class-based commands give you better inference on positional
  arguments than a `:slot` string ever will.
- **cobra** is more mature at everything softcli tries to do. It is also Go.

Where this one is genuinely different: the declaration is *reusable data*, so a
second and third consumer cost a hundred lines instead of a rewrite. If a CLI is
the only surface you will ever want, that property is worth nothing and
commander is the better choice.
