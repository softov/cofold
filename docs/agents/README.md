...
| Package | What |
| --- | --- |
| [`@cofold/agents`](../../packages/agents) | Contracts, JSON Schema validation, `createTool`, `createAgent`, `run`, `resume`, capabilities, skills and deferred tools, the step log, in-memory store, fake model. |
| [`@cofold/store-file`](../../packages/store-file) | The durable `Store` on the filesystem: sessions, runs, requests, a writer lease. |
| [`@cofold/model-openai-compat`](../../packages/model-openai-compat) | Non-streaming Chat Completions adapter (OpenRouter, LM Studio, any compatible server). |
...
Sibling packages import `@cofold/agents` through its published `exports` (`dist/`), so `typecheck` and `test` build first.
