...
The schema type is `JsonSchema` from `@facio/sdk`, the one definition every facio package shares and the one validator (`validateSchema`) they all call, and it is what `z.toJSONSchema()` writes for the ordinary shapes.
