...
The schema type is `JsonSchema` from `@cofold/sdk`, the one definition every cofold package shares and the one validator (`validateSchema`) they all call, and it is what `z.toJSONSchema()` writes for the ordinary shapes.
