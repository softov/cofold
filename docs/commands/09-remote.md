...
`surfaces.http` is `@doopx/remote`'s own key, added to `Surfaces` by declaration merging: the binding is typed where it is written and the core still knows no protocol. `@doopx/remote` reads it from **both** ends - the server routes with it, the client builds requests with it. Actions with no binding are simply not published: a local `doctor` command is nobody else's business.
