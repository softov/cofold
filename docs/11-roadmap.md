...
What is built works: `pnpm build && pnpm test` is green, and the three playground programs run end to end. What follows is honest about the rest.
...
**Answered: the HTTP binding is checked, and `remote` is a surface.** It still lives at `meta.http`, but `meta` is a `CommandMeta` that `softcli/remote` widens by declaration merging, so the binding is typed at the point it is written and the core still knows no protocol. `SurfaceFlags` gained `remote`, because publishing used to be decided by `surfaceEnabled(command, "docs")` - hiding a command from the reference withdrew it from the network. What a request carries is derived by `placementOf` rather than restated: the path takes what it names, and the rest is query for a method without a body and body for one with it, with `query` and `body` left as overrides. Still open is whether a `serve` entry point should render the registry as an HTTP API directly, rather than only describing one.
