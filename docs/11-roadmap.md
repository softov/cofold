...
**Should the HTTP binding be first-class?** It lives in `meta.http` and only `softcli/remote` understands it. If a `serve` entry point ever renders a registry as an HTTP API directly - the fourth surface - the binding stops being an extension and becomes part of the declaration.
