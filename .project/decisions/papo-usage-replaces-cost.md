...
`/chat.usage` replaces `/chat.cost`: one panel named for what it shows, from `sessionUsage` in `@cofold/agents` (per run: tokens by kind, steps, tool calls, denials; and the sums).
