...
3. **Claude is contra-validation only.** papo runs on our harness (cli/01) and on the Claude Agent SDK (cli/03); every difference is a finding presumed to be the harness's bug. The harness is never a Claude API call; no `@cofold/model-anthropic` ([decision 112](../decisions/no-anthropic-api-adapter.md)).
...
| [cli/05-papo-defaults-picker-thought](../plans/cli/05-papo-defaults-picker-thought/plan.md) | built 2026-09-16 (tasks 01-05; [implemented.md](../plans/cli/05-papo-defaults-picker-thought/implemented.md)); textui changed with it (`choices(collected)`, the thought's click and divider, the cursor bar) shipped in textui 0.6.1 (2026-09-17; papo depends on `^0.6.1` from the registry, the link is gone) | the runs by hand (two providers on the chip, arrowing through a transcript, a thought's click); `pnpm check` 67 files, 780 tests green |
...
- Two renames at the user's ask, after that check: `execute` → `executeTool` (`run/tools.ts`, `run/turn.ts`); `compact()` moved from `run/run.ts` to `run/compact.ts` (`start()` exported from `run.ts`; `index.ts` and `compact.test.ts` updated). `@cofold/agents` build, typecheck and 15 test files green; full `pnpm check` not re-run after the renames.
...
5. cli/04 is built (tasks 01-04, 2026-09-16); `@cofold/tools`' file tools changed their `subject` with it (CLI-04.6). What waits is in its `deferred.md`.

## 2026-09-18: usage and the session head (cli/06), the deferred audit

- [cli/06](../plans/cli/06-papo-usage-and-session-head/plan.md) built: `sessionUsage` in `@cofold/agents`, `Chat.usage` on both backends, `papo usage <session>`, `/chat.usage` in place of `/chat.cost`, the `ChatSessionHead` at the top of a conversation (CLI-06.1, CLI-06.2). `pnpm check` 68 files, 783 tests.
...
- `git status` in `f:\github\cofold` shows the modified and new files listed above.
