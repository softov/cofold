...
| --- | --- | --- |
| 1 | The backend is chosen per process: `config.backend: 'doopx' \| 'claude'` (default `doopx`), overridden by `--backend` / `PAPO_BACKEND`. `openPapo` builds `createChat` or `createClaudeChat`; the fronts do not know which | Two runtimes share nothing below `Chat`; switching per session would be two catalogues in one list |
