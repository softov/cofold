...
`papo` talks to the harness in this process: a screen drawn with `@textui/chat` and a shell of `@cofold/commands` actions over the same sessions on disk under `~/.cofold`.
...
- The manual run of the screen against a model server with `@cofold/tools` on (the server was unreachable on 2026-09-16).
