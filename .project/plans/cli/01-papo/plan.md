...
A person runs `papo` in a project folder and talks to an agent that runs **in this process** on `@doopx/agents`: sessions persist under `~/.doopx` per workspace, a destructive tool stops and asks, `ask_user` shows its questions, closing the terminal mid-approval loses nothing.
...
| 9 | Config through `@doopx/config` (`name: 'papo'`): `~/.config/papo/config.json`, nearest `.papo.json`, `$PAPO_CONFIG`, `--config`; `PAPO_BASE_URL`/`PAPO_API_KEY` add or replace provider `default`; `PAPO_MODEL` chooses the default model. Validated with the sdk validator through `check` (`config.permissions must be one of ...`) | Same layering as every doopx program |
| 10 | Store root `resolveHome({ name: 'doopx' })` → `~/.doopx` / `FACIO_HOME`; global skills under `<home>/skills`, workspace skills under `<workspace>/.agents/skills`; agent id `papo`; `<workspace>/AGENTS.md` appended to the instructions | The future daemon shares the store |
