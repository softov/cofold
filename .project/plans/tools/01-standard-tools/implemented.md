...
`@facio/tools` gives every agent the tools it needs to work in a repository, as capabilities: `files()`, `shell()`, `web({ search })`, `memory({ dir })`; papo turns them on from `config.tools`.
