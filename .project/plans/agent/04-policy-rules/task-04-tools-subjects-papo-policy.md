...
Rules can name what `@cofold/tools`' tools act on, and papo compiles and behaves as before on `decide()` ([117](../../../decisions/rules-are-harness-data.md); `decide()` per cli/03 F3).
...
1. `@cofold/tools`: add `subject` next to `effects` on each tool named above; `memory_read` / `memory_write` / `web_fetch` / `web_search` declare none (name-only rules).
...
- `pnpm check`.
- papo over `@cofold/agents`: `permissions: ask` asks for `read_file`; `destructive` asks for `shell_exec` only; `auto` asks nothing (manual, or the existing `chat.test.ts` scenarios).

## Resume

- **Done (2026-09-16):** `subject` next to `effects` on `shell_exec` (the `command`), `read_file`, `write_file`, `edit_file` (the `path`), `list_files`, `search_files` (the `pattern`); the memory and web tools declare none. One `subject` assertion per tool in `files.test.ts` and `shell.test.ts`. papo's `policyOf` on `decide` was already moved in task 01 (same code as step 2 here, with the comment that cli/04 task 04 replaces the three modes). READMEs: `@cofold/tools` names the subjects (files paragraph and the `shell()` section), `@cofold/papo` says each mode is one `decide` answer and rule lists are not in its configuration yet.
- **Evidence:** `pnpm --filter @cofold/tools typecheck` clean; `files.test.ts` 8 tests, `shell.test.ts` 6 tests green; papo `chat.test.ts` 13 tests green unchanged (`read_file` answers, `shell_exec` asks under `destructive` and runs under `auto`; `auto` runs a destructive tool without asking and `ask` asks for everything). `@cofold/agents` and `@cofold/papo` `tsc` report errors only in `src/testing/fake-model.ts` and `src/claude/chat.ts`, other sessions' in-progress edits; the dist still emits (no `noEmitOnError`), so the cross-package tests ran against the new types.
