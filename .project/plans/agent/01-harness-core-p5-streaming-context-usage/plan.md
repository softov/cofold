...
| --- | --- | --- |
| `RunCommand.steer`, `run.steered` | `types/command.ts`, `types/event.ts` | `run/handle.ts`, `run/steering.ts`, `@doopx/chat` (`say` during a turn) |
...
| `ReasoningEffort`, `ModelRequest.cacheKey` | `types/model.ts` | `run/context.ts`, `@doopx/model-openai-compat`, `@doopx/chat` model config |
| `ModelStreamEvent`, `ModelAdapter.stream`, `model.delta.kind` | `types/model.ts`, `types/event.ts` | `run/turn.ts` (`callModel`), `testing/fake-model.ts`, `@doopx/model-openai-compat` (`stream.ts`), papo (`cli` plan) |
| `ModelPricing`, `ModelAdapter.pricing`, `Usage.cacheWriteTokens`, `RunRecord.cost`, `RunOutcome.cost`, `Limits.maxCost`, `StopReason.max_cost` | `types/model.ts`, `types/provider.ts`, `types/store.ts`, `types/outcome.ts`, `types/limits.ts` | `model/cost.ts`, `run/turn.ts`, `run/resume.ts`, every store (`memory.ts`, `store-file`), `@doopx/model-openai-compat` (`fromWireModel`), papo's turn usage |
...
1. A steer that arrives while the last model step is in flight is rejected, not silently dropped, and not silently turned into a new run: the consumer decides (`@doopx/chat` resends it as a new `say`). Decision 95 makes this explicit; the alternative (the harness starting a new run) would hide a run boundary from the store.
...
- **Done so far:** Tasks 1-4 built 2026-09-16 (decisions 95-101 in code: `run/steering.ts`, `run/handle.ts`, `run/run.ts`, `run/resume.ts`, `run/turn.ts`, `run/tools.ts`, `types/*`, `@doopx/model-openai-compat` `index.ts` / `wire.ts` / `types/options.ts`; tests `run/steering.test.ts`, `run/hook-stop.test.ts`, `wire.test.ts`; `examples/agents/steer.ts`, `adapter-smoke.ts --effort`); Task 6 built 2026-09-16; Task 5 built 2026-09-16 (`run/turn.ts` `callModel`, `types/model.ts` `ModelStreamEvent` / `stream?`, `testing/fake-model.ts` `stream: true`, `run/stream.test.ts`; `@doopx/model-openai-compat` `sse.ts`, `stream.ts`, `stream()` on the adapter, `DEFAULT_FEATURES.streaming: true`, `sse.test.ts`, `stream.test.ts`); Task 7 built 2026-09-16 (`model/cost.ts`, `tally()` in `run/turn.ts`, `maxCost`, `RunTally`, `RunRecord.cost`, both stores, `run/cost.test.ts`; openai-compat `pricing` and catalogue cache rates); Task 8 built 2026-09-16 (`compactKeepTokens` and `splitTail`, `contextOf` exported, `messageId`, `message/markers.ts` and `interrupt()`, cancel-while-waiting denies, `context.compacted` `kept` / `afterTokens`; `run/interrupt.test.ts`, `compact.test.ts`, `run.test.ts`, `resume.test.ts`). Every task done; the plan is built: see [implemented.md](implemented.md).
- **Planned 2026-09-16:** Tasks 5, 7, 8 in full (decisions 102, 104, 105, 108, 109, 112, 120; findings F1, F2, F4, F6, F7 and the spec items are task steps, not decisions). The former task 08 (an Anthropic Messages API adapter) is void: no `@doopx/model-anthropic`, no `agent/04` adapter plan (decision 112).
...
- [x] A streamed answer arrives as persisted `model.delta` events; an interrupted stream leaves no tool call to execute (2026-09-16: `run/stream.test.ts` cases 1, 4, 7; `@doopx/agents` 17 test files, `@doopx/model-openai-compat` 4).
