---
title: AGENT-01 - Harness core (parent)
domain: agent
status: active
priority: high
created: 2026-09-13
...
Build `@cofold/agents`: a TypeScript agent runtime that conducts a conversation with a model, offers tools, executes authorized tool calls, assembles context, manages one turn as a run, and exposes what happens through a run handle.
...
- `.project/specs/agent-harness-spec.md` - the specification; every section maps to a phase below.
...
- `F:\github\cofold\src\core\command.ts:192-270` - `Field = JsonSchema & { cli?, env? }`, `ActionDefinition { id, summary, input, required, surfaces, needs, refine?: StandardSchemaV1, run(context) }`.
- `F:\github\cofold\package.json` - tsc build, vitest, `engines.node >= 22`, zero runtime deps, `exports` map with sub-paths; the style to match.
...
- `.project/research/agent-harness-survey.md` - the survey; sections 1-12 list every feature considered in or out of scope.

### Searches performed

- `ls` of the agents repository - it did not exist beyond `roadmap/`; nothing to reuse in-tree.
- `npm view @cofold/core` - 404; the `@cofold` scope is unused (the user owns `cofold`).
- `grep -n "export interface Agent\|export interface Session" ahpd/packages/sdk/src/types/*.ts` - located the two contracts above.
- `grep -n "StandardSchemaV1\|export type Field" cofold/src/core/*.ts` - cofold validates JSON Schema fields itself and accepts an optional Standard Schema `refine`.
...
- `cofold/src/core/command.ts` single-object definitions with JSON Schema input; `createRegistry()` factory style.
...
| --- | --- | --- |
| 1 | Packages published as `@cofold/*`, core is `@cofold/agents`. (Started in its own repository; since 2026-09-16 part of the `cofold` workspace) | User. The command framework is `@cofold/commands`; `cofold` is reserved for the program |
...
| 6 | `@cofold/agents` has zero runtime dependencies: own JSON Schema validator subset, native `fetch`, `crypto.randomUUID` | User (asked 2026-09-13), matches cofold |
...
| 13 | First model adapter is non-streaming Chat Completions (`@cofold/model-openai-compat`) covering OpenRouter and LM Studio; adapters report `features` and reject unsupported required features | Spec "Model adapters" |
| 14 | Run outcome statuses: `completed`, `awaiting`, `stopped`, `cancelled`, `failed`; never a single "done" | Spec "Turn lifecycle" |
| 15 | `@cofold/agents` never imports `cofold`; `fromCofoldAction()` is a separate adapter (later plan) | User agreed |
...
| 28 | First durable store is `@cofold/store-file` (JSONL per session and run, `writer.lock` as the fence). SQLite and Durable Object stores are later packages behind the same `Store` contract | User: "files is first. sqlite is pluggable" |
| 29 | **Workspace** is a host-supplied session partition key (`SessionRecord.workspace`, `RunArgs.workspace`): the CLI passes `process.cwd()`, the AHP transport passes `SessionOptions.cwd`, chat hosts pass nothing. The harness owns the concept, its kv scope (`{ kind: 'workspace' }`), `sessions.list({ workspace })`, and the on-disk slug; hosts own only the root and the key. Nothing in `@cofold/agents` reads `os.homedir()` or `process.cwd()` | User (2026-09-14): mirror the `<home>/projects/<cwd-slug>/` layout coding harnesses use, but keep the harness host-agnostic so CLI and ahpd share one on-disk state |
...
- **Layer responsibilities.**
  - `@cofold/agents`: contracts, JSON Schema validation, memory store, fake model, `createAgent` (resolves options into a frozen `Agent` value), `run` / `resume` (the loop and run handle), step log, context assembler (recent history only).
  - `@cofold/model-openai-compat`: HTTP, headers, retries, response decoding, feature flags.
  - `@cofold/store-file` (p3): first durable implementation of the `Store` contract (JSONL per session and run, lock file as writer fence). `@cofold/store-sqlite` and a Durable Object store come later behind the same contract.
  - `@cofold/transport-ahp` (p4): `Agent`/`Session` from `@ahpd/sdk` on top of `createAgent` + `run()`.
  - `examples/`: the "harness uses the agent" hosts.
- **Source-of-truth files.** `packages/agents/src/types/{message,tool,model,event,command,outcome,store,hooks,agent,run}.ts`. Everything else imports from `@cofold/agents`.
...
| p3 | (shipped, plan removed) | `@cofold/store-file`, paused approvals with durable `requestId`, `resume()`, writer fence; prove pause -> lose observer -> resume without double execution | Shipped 2026-09-16 |
...
Code mode (`run_code` + generated `.d.ts` + isolate), guardrail package, `agentAsTool`, `createNetwork`, memory hooks, SQLite and Durable Object stores, JSON-RPC / WS / HTTP / MCP-server transports, `fromCofoldAction()`, parallel tool execution, session fork / rewind (contract slots only in p3).
Each gets its own plan in its domain folder.

## Cross-layer consistency

One concept, one type, from `packages/agents/src/types/`.
Adapters, stores, transports, and examples import `@cofold/agents` types and never redeclare `Message`, `Tool`, `RunEvent`, `RunCommand`, `RunOutcome`, `Store`.
`@cofold/transport-ahp` maps to `@ahpd/sdk` types at its boundary only.
...
- The in-memory store is the only store until p3; every p2 test must also pass against `@cofold/store-file` once it exists (p3 re-runs the p2 suite), and against any later store.

## Resume state

- **Done so far:** p1-p3 shipped (contracts, validator now in `@cofold/sdk`, loop, run handle, file store, durable approvals, resume, writer fence); p5 built in full 2026-09-16 (steering, hook stop, effort levels, dynamic key, cache key, compaction, streaming, usage accounting and cost, the papo contra-validation findings F1, F2, F4, F6, F7); every package's types in `src/types/`.
...
- **Open questions:** none.
- **Watch out for:** the `@cofold` npm scope must be claimed by the user before the first publish; the package names in this plan assume it.
...
- [ ] No contract drift: every package imports shapes from `@cofold/agents`.
