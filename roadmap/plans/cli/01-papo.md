...
Dependencies: ../agent/01-harness-core.md (p3 shipped); textui `@textui/chat` 0.5.0+ (linked from the sibling checkout until 0.6.0 is published)
Reference: ./00-cli.md
-->
...
A person runs `papo` in a project folder and talks to an agent that runs **in this process** on `@facio/agents`: sessions persist under `~/.facio` per workspace, a destructive tool stops and asks, `ask_user` shows its questions, closing the terminal mid-approval loses nothing.
The same conversations are reachable from a shell (`papo say`, `papo approve`, `papo session list --json`), and those commands are `@facio/commands` actions, so they are the daemon's MCP and HTTP surface the day it wants them, and what a future ahpc can drive instead of its hand-written flag tables.
...
| --- | --- | --- |
| 1 | Package `@facio/papo` in `packages/papo/`, binary `papo`; depends on `@facio/{agents,commands,config,model-openai-compat,sdk,store-file,terminal}` (`workspace:^`) and `@textui/{chat,core,terminal,widgets}` as `link:../../../textui/packages/<name>` until textui 0.6.0 is on npm | User (2026-09-16): papo lives in facio packages; textui is published, facio depends on it, never the reverse |
...
| 8 | `ApprovalPayload` and `InputPayload` moved to `@facio/agents` `types/store.ts` and exported | papo reads `PendingRequest.payload`; one definition |
| 9 | Config through `@facio/config` (`name: 'papo'`): `~/.config/papo/config.json`, nearest `.papo.json`, `$PAPO_CONFIG`, `--config`; `PAPO_BASE_URL`/`PAPO_API_KEY` add or replace provider `default`; `PAPO_MODEL`/`--model` choose. Validated with the sdk validator through `check` (`config.permissions must be one of ...`) | Same layering as every facio program |
| 10 | Store root `resolveHome({ name: 'facio' })` → `~/.facio` / `FACIO_HOME`; global skills under `<home>/skills`, workspace skills under `<workspace>/.agents/skills`; agent id `papo`; `<workspace>/AGENTS.md` appended to the instructions | The future daemon shares the store |
...
- `packages/papo/` as decision 1; `pnpm check` green: 30 papo tests (service, shell, projection, config, screen through `@textui/testing`), 4 new store-conformance and command cases.
- Harness: `Store.sessions.delete` (decision 7), payload types public (decision 8), variadic slot coercion (decision 14).
- `roadmap/plans/index.md`, `cli/00-cli.md`, `CLAUDE.md`, root `README.md` updated.
