...
| 9 | Config through `@facio/config` (`name: 'papo'`): `~/.config/papo/config.json`, nearest `.papo.json`, `$PAPO_CONFIG`, `--config`; `PAPO_BASE_URL`/`PAPO_API_KEY` add or replace provider `default`; `PAPO_MODEL` chooses the default model. Validated with the sdk validator through `check` (`config.permissions must be one of ...`) | Same layering as every facio program |
