...
| 3 | Framework split, one package per sub-path, names: `@facio/core` (`src/core` + `src/index.ts`), `@facio/terminal` (`src/cli`), `@facio/mcp` (`src/mcp`, sub-paths `./stdio`, `./server`), `@facio/remote`, `@facio/config`, `@facio/yaml`, `@facio/docs`. Six depend on `@facio/core` (`workspace:^`); `@facio/yaml` depends on nothing; `@facio/mcp` alone carries the optional peer `@modelcontextprotocol/sdk` | Import graph (Recon); user: `cli` would read as a CLI program, which it is not; `src/cli/index.ts` calls itself "the terminal rendering" |
| 4 | **Proposed, user to confirm:** the contract-and-registry package is `@facio/core`, not `@facio/sdk` or `@facio/base`. It is runtime (the registry resolves capabilities, `coerce` validates), not contracts only; `@facio/sdk` stays free for a later contract-only package shared by commands and agents (the `action` → `Tool` bridge) | `src/index.ts` header; opendoop's `packages/sdk` is contracts only, which this is not |
| 5 | Versions after Task 3: every framework package starts at `0.2.0` (above the last `facio` release); agents packages keep their versions. `facio@0.1.x` on npm is deprecated with `npm deprecate facio@"<1" "split into @facio/core, @facio/terminal, @facio/mcp, @facio/remote, @facio/config, @facio/yaml, @facio/docs"` when the first `@facio/*` framework package is published (user runs it; the `@facio` npm org must be the user's) | (defaulted) |
...
facio/
  ...
  packages/
    core/  terminal/  mcp/  remote/  config/  yaml/  docs/     (Task 3; Task 1-2: packages/facio/)
...
| `@facio/core` exports (`JsonSchema`, `Registry`, …) | `packages/core/src/index.ts` | terminal, mcp, remote, config, docs, examples/commands, later `@facio/agents`' action→Tool bridge |
...
4. **`@facio/core` is the name proposed, not confirmed** (decision 4). Renaming later is a search-replace over the same list Task 3 already touches, so the cost of deciding after Task 2 is small; the cost of deciding after publishing is a second deprecate.

## Resume state

- **Done so far:** plan written 2026-09-16. Nothing moved.
- **Next action:** user confirms decision 4 (`core` vs `sdk` / `base`), then Task 1 by the dev session (with the git moves handed to the user).
- **Open questions:** decision 4 only.
...
- [ ] After Task 1: `pnpm check`, `pnpm examples`, petshop smoke, `pnpm --filter facio pack` contents.
