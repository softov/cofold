...
import { CofoldError, compact, type Command, type RequestContext, type Runner } from "@cofold/commands";
...
/** SDK integration over Cofold's JSON Schema descriptors; no schema translation into Zod. */
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request, extra) => {
    ...
    try {
      ...
      const message = extra.signal.aborted ? "Tool cancelled"
        : error instanceof CofoldError && ["argument", "authorization", "conflict"].includes(error.kind)
