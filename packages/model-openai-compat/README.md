...
- Reasoning: with `features.reasoning: true`, `params.reasoning.effort` is sent as `reasoning_effort` and, as soon as `maxTokens` is set, as OpenRouter's `reasoning: { effort, max_tokens }`. On reply, `message.reasoning` (OpenRouter), `message.reasoning_content` (DeepSeek, LM Studio) or a leading `<think>...</think>` block in `content` becomes a `reasoning` part placed before the text; `usage.reasoningTokens` comes from `completion_tokens_details.reasoning_tokens`. Reasoning parts are never sent back.
