...
describe('loadConfig', () => {
  it('has defaults and no provider until one is written', () => {
    ...
    expect(config).toMatchObject({ backend: 'facio', providers: [], permissions: 'default', reasoning: 'off', theme: 'paper', shell: 'workbench' });
...
describe('model references', () => {
  ...
  it('finds the provider by id and says which are configured when it is missing', () => {
    const config = { backend: 'facio' as const, providers: [{ id: 'a', baseUrl: 'http://a' }, { id: 'b', baseUrl: 'http://b' }], permissions: 'acceptEdits' as const, reasoning: 'off' as const, instructions: '', tools: { files: false, shell: false, web: false, memory: false }, context: { maxTokens: 32_000, autoCompact: false }, theme: 'paper', shell: 'workbench' };
