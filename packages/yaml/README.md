# @facio/yaml

**A documented YAML subset as plain data.**

Parsing with source-aware errors, and document loading with local and remote references, JSON pointers and bounded reads. No dependencies, no I/O in the parser.

```ts
import { parseYaml, loadYaml } from "@facio/yaml";
```

Part of the [facio](https://github.com/softov/facio) family.
The manual page is [`docs/commands/11-yaml.md`](../../docs/commands/11-yaml.md); the whole framework is described in [`@facio/commands`](../commands).

## License

MIT
