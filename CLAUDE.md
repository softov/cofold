...
- **Zero third-party runtime dependencies per package.** Own JSON Schema validator, native `fetch`, `crypto.randomUUID`. The framework runs unmodified on Node, Bun and Deno. `@facio/mcp`'s SDK server is the one optional peer. papo depends on `@textui/*` (published; linked from the sibling checkout until 0.6.0), and facio never depends the other way round.
